...
final class LocationSimulationService {
    ...
    private func setLocationSync(_ coordinate: CLLocationCoordinate2D, pairingFile: URL) throws {
        ...
        guard targetIP.withCString({ inet_pton(AF_INET, $0, &address.sin_addr) }) == 1 else { throw ServiceError.invalidIP }
        ...
        let createError = withUnsafePointer(to: &address) { pointer in
            pointer.withMemoryRebound(to: sockaddr.self, capacity: 1) {
                tunnel_create_rppairing($0, socklen_t(MemoryLayout<sockaddr_in>.stride), "LocationSpoof", pairing, nil, nil, &adapter, &handshake)
            }
        }
        if let createError { idevice_error_free(createError); cleanup(); throw ServiceError.providerCreate }
        if let error = remote_server_connect_rsd(adapter, handshake, &remoteServer) {
            idevice_error_free(error); cleanup(); throw ServiceError.remoteServer
        }
        if let error = location_simulation_new(remoteServer, &simulation) {
            idevice_error_free(error); cleanup(); throw ServiceError.simulationCreate
        }
        remoteServer = nil
        if let error = location_simulation_set(simulation, coordinate.latitude, coordinate.longitude) {
            idevice_error_free(error); cleanup(); throw ServiceError.setFailed
        }
    }
